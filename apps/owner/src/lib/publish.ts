...
// out of that `'use server'` module so it can take an injected `MessagesClient` for the integration
// test. Generates the 7 audience copy bundles, validates the whole set, and commits transactionally
// — ALL-OR-NOTHING: on any failure (including a network error or a single variant that won't
// validate after retries) it touches neither the existing `PageVariant` rows nor the published
// state, reverting `publishState` to its prior value; a failed re-publish leaves the live page
// intact. The "in-flight" marker (`publishState:'publishing'`) is a separate non-transactional
// write; the next publish reverts it / a re-publish overwrites it.
...
const AI_NOT_CONFIGURED_MESSAGE = 'AI is not configured — set ANTHROPIC_API_KEY to publish.';
...
export async function runPublishPipeline(
  ...
  try {
    results = await generateAllVariants(
      { venueName: venue.name, description: venue.description },
