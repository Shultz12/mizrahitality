...
- **Google Gemini** (`gemini-2.5-flash-lite` via `@google/genai` — Google's lowest-latency structured-output-capable model) — used only at publish time, only on the description **text**, never on the image, never in the page-serving request path. Per-variant copy uses Gemini's structured output (`responseMimeType: 'application/json'` + `responseSchema`). Billing is enabled on the key, so there is no inter-call pacing — calls fire back-to-back. A publish runs as an in-memory `PublishJob` the client polls so the Re-publish button can show live "X / 7" progress.
