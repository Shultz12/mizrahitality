...
Mizrahitality is a landing-page platform that turns two simple inputs — a rough description and one photo — into a fast, server-rendered venue page that quietly tailors itself to whoever is looking at it.
Unlike general-purpose website builders that drown non-technical users in options, Mizrahitality ships exactly one page — a Rich Text slot the AI writes and an Image slot the owner's photo fills — with no editor, no layout, and nothing to compose, and hands back a live analytics dashboard instead of a blank canvas.
...
|-----------|-------------|-------------|
| Hospitality venue owner | Owner/operator of a small venue; technophobic, time-poor; signs up with email, password, and a venue slug | Get a working, good-looking venue page live in minutes without making any technical decisions, and see whether it's working |
...
| API consumer (Mizrahitality-customer) | The server-rendered visitor-facing site that fetches rendered pages and reports visit/hover/click events back over a per-venue API key | A stable REST contract: request a page by venue slug + visitor type, receive a fully-formed page; post analytics events reliably |
...
## Differentiation
- **One page, two slot types, on purpose** — a Rich Text slot and an Image slot. No layout engine, no component library, no rich-text editor, no image placement, no drag-and-drop, nothing to get wrong; the owner just writes a description and picks a photo.
- **AI does the writing and the targeting** — the owner supplies a rough description and one photo; the platform polishes the copy (owner-approved), then authors it into the Rich Text slot of a pre-designed template per visitor type (gender × age group), plus a neutral default, each tailored by that template's per-audience rules, while the owner's photo fills the Image slot. Templates and styling are designed up front; the AI fills the copy, it doesn't invent layout or styling.
- **Server-side rendered, audience-aware delivery** — the public page is composed on the server per visitor type and served fully-formed; the visitor site is a thin SSR client over a clean REST API.
